...
const openProduct=id=>{const item=itemById[id];if(!item)return;document.querySelector('#product-detail').innerHTML=`<div class="product-detail-grid"><div class="product-detail-image" style="background-image:url('assets/${item.img}')" role="img" aria-label="${item.name}"></div><div class="product-detail-copy"><p class="dialog-kicker">${item.series} / ${item.stock}</p><h2 id="product-dialog-title">${item.name}</h2><p>${item.copy}</p><p><b>触感</b> ${item.touch}<br><b>価格帯</b> ${price(item)}<br><b>レア度</b> ${stars(item)}</p><div class="card-actions"><button class="cart-add" type="button" data-cart-add="${item.id}" ${item.stock==='SOLD OUT'?'disabled':''}>${item.stock==='SOLD OUT'?'SOLD OUT':'CART +'}</button><button class="fav-toggle" type="button" data-favorite-id="${item.id}" aria-label="${item.name}をお気に入りに追加" aria-pressed="${favorites.has(item.id)}">♡</button></div></div></div>`;renderFavorites();dialogOpen(document.querySelector('#product-dialog'))};
